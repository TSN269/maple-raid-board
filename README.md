...
UI-5.6 是以 UI-5.5 為基礎，新增與調整：

```text
1. 新增突襲場次支援直接帶入隊伍收藏名單
   - 新增突襲場次視窗內新增「隊伍收藏名單」區塊
   - 可按「帶入隊伍收藏」選擇左側隊伍收藏中的隊伍
   - 可一次選擇多個收藏隊伍

2. 建立場次後自動加入成員
   - 選取的隊伍收藏名單會在建立場次後自動寫入新場次成員
   - 會保留成員名稱、職業、等級、角色定位、備註
   - 會依照收藏隊伍編號帶入隊伍欄位

3. 名額上限自動調整
   - 若帶入隊伍收藏人數大於目前名額
   - 系統會自動把名額上限調整到足夠容納的人數
   - 上限仍維持最多 18 人
...
UI-5.6 **沒有修改 Supabase schema / RPC**。  
如果已經執行過 UI-3.3 SQLFIX1 的 SQL，升級 UI-5.6 不需要重跑 SQL。
