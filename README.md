...
UI-5.7 是以 UI-5.6 為基礎，新增與調整：

```text
1. 練功效率偵測 Debug 位置調整
   - 原本 Debug 顯示在標題下方獨立區塊
   - 改為移到狀態標籤右邊
   - 位置會在「未啟動 / 統計中 / OCR 自動辨識中」右側
...
UI-5.7 **沒有修改 Supabase schema / RPC**。  
如果已經執行過 UI-3.3 SQLFIX1 的 SQL，升級 UI-5.7 不需要重跑 SQL。
