# Maple Raid Board — TSN UI-6.6

> 版本基準：UI-6.6  
> GitHub 帳號：TSN269  
> 專案用途：楓之谷 / Artale 類型突襲報名看板 + 羅茱跳台協作工具 + 練功效率偵測 + 隊伍收藏 + 遊戲id / 特徵碼紀錄  
...
本專案從「突襲報名看板」逐步擴充成整合型 Maple / Artale 工具站。  
截至 UI-6.6，主要功能分成五大區塊：

### A. 突襲場次管理

提供 Boss 團報名、團長管理、邀請碼、隊伍配置、角色定位需求、通知中心與防洗版功能。
...
提供練功 EXP 分析、OCR 自動辨識、統計卡、折線圖、統計資訊紀錄與圖片擷取功能。
...
- 螢幕擷取後自動 OCR 辨識 EXP
- 自動抓取 OCR 裁切區
- Debug 模式可顯示 OCR 細節與手動框選裁切區
- 手動修正目前 EXP
- 手動加入紀錄
- 輸入當前等級後自動帶入 Artale 升下一级所需经验
- EXP / 分折線圖
- OCR 極端誤判值過濾
- 擷取統計資訊圖片
- 統計圖片優先複製到剪貼簿
...
- 左側導覽新增「隊伍收藏」
- 已結束場次可在隊伍配置欄收藏隊伍 1 / 2 / 3
- 隊伍收藏顯示場次名稱、Boss、日期時間、團長、隊伍編號與隊員名單
- 同一場次同一隊重複收藏會覆蓋舊收藏
- 新增突襲場次時可直接帶入隊伍收藏
- 建立新場次後自動加入收藏隊伍成員
- 名額上限會依帶入成員數自動調整，最多 18 人

### E. 遊戲id / 特徵碼紀錄

提供本機保存的遊戲id / 特徵碼管理與報名欄位連動。
...
- 可匯入 / 匯出遊戲id / 特徵碼紀錄
- 我要報名角色名稱若有紀錄會改為下拉選單
- 新增突襲場次團長角色名若有紀錄會改為下拉選單
- 無紀錄時顯示「若有紀錄『遊戲id / 特徵碼』可下拉選取」提示
...
| 螢幕擷取 | Browser MediaDevices / getDisplayMedia |
| 圖片輸出 | Canvas / PNG |
...
如果是從 UI-5.9 以前升級，需確認已執行：

```text
supabase/ui-5-9-sqlfix1-role-options.sql
```

該 SQLFIX 修正角色定位白名單，允許：
...
如果已經執行過 **UI-5.9 SQLFIX1** 或之後版本的完整 schema，UI-6.6 不需要重跑 SQL。
...
|---|---|
| 首頁 | 顯示突襲場次清單與目前選取團 |
| 楓突襲 | 顯示突襲詳細內容、公告、隊伍配置、團長管理、隊伍收藏 |
...
| 設定 | 查看並匯入 / 匯出團長管理碼、邀請碼、帶邀請碼團連結 |

右上蘑菇 Logo：

```text
點擊可開啟「遊戲id / 特徵碼紀錄」小頁面
```
...
```text
1. 特徵碼允許 1～6 位英數字元
2. 遊戲id 最多 9 字，避免報名角色名稱超過 16 字限制
3. 最多保存 10 筆
4. 資料保存於目前瀏覽器 localStorage
```
...
3. 無紀錄時
   - 顯示「若有紀錄『遊戲id / 特徵碼』可下拉選取」提示
```

### 匯入 / 匯出

```text
1. 可匯出遊戲id / 特徵碼紀錄為 JSON
2. 可匯入 JSON 紀錄
3. 匯入後仍最多保留 10 筆
```
...
```text
1. 開始分析(F8)：開始螢幕擷取、OCR 與計時
2. 暫停分析 (F9)：暫停 OCR 與計時
3. 繼續分析 (F9)：恢復 OCR 與計時
4. 停止分析 (F10)：停止 OCR 與計時
5. 重置：清空目前練功效率紀錄
```
...
1. 按下開始分析後會自動啟動畫面擷取
2. 自動抓取 EXP 裁切區
3. 預設裁切區為 X 50.4 / Y 93.6 / 寬 13 / 高 6.4
4. Debug 勾選後才顯示手動框選、儲存預設、清除預設
5. Debug 勾選後才顯示 OCR 間隔、成功、失敗、最近辨識與座標資訊
```

### EXP 與等級

```text
1. 手動修正目前 EXP 可修正 OCR 誤判
2. 可手動加入紀錄
3. 輸入當前等級後，自動帶入 Artale 經驗表的「升下一级所需经验」
4. 目前 EXP 會換算成目前等級百分比
5. EXP 增量會顯示對應百分比
```
...
3. 統計時間與開始分析時間
4. EXP 累積 10 分 / 60 分
5. 預估 10 分、近 10 分、最高 10 分
6. 預估 60 分、近 60 分、最高 60 分
7. 預估百分比 1 / 10 / 60 分
8. 預估升級時間與升級後等級
```
...
```text
1. 下方趨勢圖以 EXP / 分折線圖呈現
2. OCR 極端誤判值會被忽略，不加入趨勢圖資料
3. 可按「擷取統計資訊」輸出目前統計圖片
4. 統計圖片優先複製到剪貼簿
5. 瀏覽器不支援時退回分享視窗或下載 PNG
```

### 統計資訊紀錄

```text
1. 按「紀錄統計資訊」保存當下統計區內容
2. 最多保存最近 10 次
3. 按「檢視之前統計資訊」開啟紀錄小頁面
4. 點擊紀錄時間可查看該時間點資訊
5. 每筆紀錄可單獨清除
6. 統計資訊紀錄小頁面也可按「擷取統計資訊」
7. 歷史紀錄擷取圖片時，圖片副標題顯示該筆紀錄時間
```
...
```text
1. 只有已結束突襲場次會顯示「收藏隊伍名單」
2. 收藏按鈕位於楓突襲的隊伍配置欄
3. 若場次有隊伍 1～3，會顯示收藏隊伍 1 / 2 / 3
4. 空隊伍不可收藏
```
...
3. 可從收藏卡片回到原場次
4. 可移除單筆收藏
5. 同一場次同一隊重複收藏時會覆蓋舊收藏
```

### 新增突襲時帶入

```text
1. 新增突襲場次視窗內可按「帶入隊伍收藏」
2. 可選擇一個或多個收藏隊伍
3. 建立場次後自動加入收藏隊伍的成員
4. 帶入資料包含名稱、職業、等級、角色定位、備註與隊伍編號
5. 若帶入人數大於名額上限，會自動調整名額，上限仍為 18 人
```
...
1. 匯出管理碼紀錄
2. 匯入管理碼紀錄
3. 匯出內容包含團長管理碼、團邀請碼與帶邀請碼團連結資訊
4. 匯入後會回寫到本機 localStorage
```
...
6. 隊伍收藏保存於目前瀏覽器 localStorage
7. 遊戲id / 特徵碼紀錄保存於目前瀏覽器 localStorage
8. 練功效率偵測需要瀏覽器允許螢幕擷取權限
9. OCR 辨識效果受遊戲解析度、字體清晰度與裁切區影響
10. 統計圖片複製到剪貼簿需瀏覽器支援 Clipboard API
11. 從舊版升級時，請確認 schema.sql 已執行到 UI-5.9 SQLFIX1 或之後版本
```
...
9. 其他瀏覽器加入羅茱房間是否即時同步
10. 角色被選後，其他人是否不能再選
11. 羅茱快捷鍵 1 / 2 / 3 / 4 是否能填入下一層
12. 羅茱 10 層填滿後快捷鍵是否停止
13. 羅茱上次路徑是否自動保存
14. 羅茱閒置 1 小時是否強制退出並清空
15. 點擊右上蘑菇 Logo 是否可開啟遊戲id / 特徵碼紀錄
16. 特徵碼是否允許 1～6 位英數字元
17. 遊戲id / 特徵碼紀錄是否可匯入 / 匯出
18. 我要報名角色名稱是否會依紀錄改為下拉選單
19. 新增突襲團長角色名是否會依紀錄改為下拉選單
20. HARD 鐘王是否顯示控時與清球
21. NORMAL 鐘王是否只顯示清球、不顯示控時
22. 殘暴炎魔是否顯示清魔靈
23. 隊伍角色定位需求是否可保存大法、清球、清魔靈
24. 練功效率 F8 是否能開始分析
25. 練功效率 F9 是否能暫停 / 繼續分析
26. 練功效率 F10 是否能停止分析
27. 練功效率是否能 OCR 辨識 EXP
28. Debug 勾選後是否才顯示 OCR 裁切與紀錄細節
29. EXP / 分折線圖是否會顯示有效資料
30. OCR 極端誤判值是否不加入趨勢圖資料
31. 擷取統計資訊是否能複製圖片或下載 PNG
32. 紀錄統計資訊是否只紀錄、不自動彈窗
33. 檢視之前統計資訊是否能打開紀錄小頁面
34. 統計資訊紀錄是否可單筆清除
35. 統計資訊紀錄小頁面是否可擷取歷史紀錄圖片
36. 已結束場次是否能收藏隊伍名單
37. 左側隊伍收藏是否顯示收藏的隊伍
38. 新增突襲場次是否能帶入隊伍收藏名單
39. 設定頁面是否可匯入 / 匯出管理碼紀錄
```
...
```bash
git add README.md
git commit -m "update readme for ui 6.6"
git push
```
