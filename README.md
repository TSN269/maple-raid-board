...
UI-5.5 是以 UI-5.4 為基礎，新增與調整：

```text
1. 左側導覽新增「隊伍收藏」
   - 位置放在「團連結收藏」下方
   - 顯示從楓突襲隊伍配置收藏的隊伍名單
   - 收藏資料保存於目前瀏覽器 localStorage

2. 已結束突襲場次可收藏隊伍名單
   - 在「楓突襲」的「隊伍配置」欄新增「收藏隊伍名單」區塊
   - 僅在突襲狀態為「已結束」時顯示
   - 若場次有隊伍 1 到隊伍 3，就會顯示可收藏隊伍 1 / 2 / 3 的按鈕
...
UI-5.5 **沒有修改 Supabase schema / RPC**。  
如果已經執行過 UI-3.3 SQLFIX1 的 SQL，升級 UI-5.5 不需要重跑 SQL。
