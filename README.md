...
UI-7.1 **沒有修改 Supabase schema / RPC**。  
如果已經執行過 UI-6.7 SQLFIX，升級 UI-7.1 不需要再執行 SQL。
...
| 通知 | 開團提醒、狀態變更、候補轉正 |
| 羅茱工具 | 跳台房間與路徑協作工具 |
| 設定 | 查看並匯入 / 匯出本機保存的團長管理碼、邀請碼、帶邀請碼連結 |
...
13. 上次路徑是否自動保存
14. 閒置 1 小時是否強制退出並清空
15. 點擊右上 Logo 是否可開啟遊戲id / 特徵碼紀錄
16. 特徵碼是否限制為 6 位英數字元
17. 我要報名角色名稱是否會依紀錄改為下拉選單
18. HARD 鐘王是否顯示控時與清球
19. NORMAL 鐘王是否只顯示清球、不顯示控時
20. 殘暴炎魔是否顯示清魔靈
```
